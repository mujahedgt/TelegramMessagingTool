...
- Runtime composition now builds runtime services through `Runtime/AppServicesBuilder.cs`, command registration through `Runtime/CommandRouterFactory.cs`, and Telegram update handling through `Runtime/TelegramUpdateHandler.cs`, keeping `Program.cs` thinner while preserving behavior
