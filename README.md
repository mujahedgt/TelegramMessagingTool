...
- Vector-store abstraction foundation for future scalable document retrieval: `IVectorStore`, `DocumentVector`, `VectorSearchResult`, and a tested `LocalJsonVectorStore` fallback. Current document Q&A behavior still uses the existing SQL `DocumentChunk.EmbeddingJson` path until provider wiring is enabled later.
...
| `VECTOR_STORE_PROVIDER` | No | `embedding_json` | Reserved for future vector-store provider selection. Current runtime keeps using SQL `DocumentChunk.EmbeddingJson`; the local JSON vector-store implementation is staged and tested but not wired into document retrieval yet. |
