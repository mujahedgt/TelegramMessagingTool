...
- Trusted local plugin loading is available behind `ENABLE_PLUGINS=true`; plugin DLLs must live under `PLUGIN_DIRECTORY`, pass manifest validation, expose allowlisted `IAgentTool` names, avoid duplicates, and appear in `/tools` with `source: plugin:<id>`
