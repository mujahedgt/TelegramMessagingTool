...
- File commands: `/files`, `/images`, `/describeimage <id>`, `/imageprompt <image-file-id|idea>`, `/voicefiles`, `/transcribe <id>`, `/voicebrief <id>`, `/voiceplan <id>`, `/transcriptinsights <id>`, `/transcripttasks <id>`, `/speaktext <text>`, `/sendaudio <id>`, `/exportchat [txt|docx|pdf] [last N]`, `/exportdata [json]`, `/readfile <id>`, `/createfile <filename> <content>`, admin-only local import via `/importfiles` and `/importfile <filename>`, and admin-approved `/deletefile <id>`
