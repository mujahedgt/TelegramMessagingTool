...
| `ENABLE_STREAMING_RESPONSES` | No | `false` | Reserved rollout flag for future Telegram edit-in-place streamed responses. Current implementation includes Ollama streaming parsing/client support, `StreamingResponseService` fallback behavior, throttled draft-edit infrastructure, and an agent-level safety boundary that refuses to stream when tools are available. Normal Telegram runtime replies still use the non-streaming path until final runtime wiring is enabled in a later phase. |
