...
| `ENABLE_TEXT_TO_SPEECH` | No | `false` | If true, `/speaktext <text>` can execute the configured trusted local TTS command. Keep false until `TEXT_TO_SPEECH_COMMAND` is configured and tested locally. Generated audio is saved only; it is not sent automatically. |
...
| `TEXT_TO_SPEECH_OUTPUT_EXTENSION` | No | `.mp3` | Expected provider output extension: `.mp3`, `.wav`, `.m4a`, `.ogg`, `.oga`, `.opus`, or `.flac`. |
