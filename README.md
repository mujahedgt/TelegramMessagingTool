...
| `ENABLE_STREAMING_RESPONSES` | No | `false` | Reserved rollout flag for future Telegram edit-in-place streamed responses. Current implementation includes Ollama streaming parsing/client support plus `StreamingResponseService` fallback behavior, but normal runtime replies still use the non-streaming path until Telegram edit-in-place wiring is enabled in a later phase. |
